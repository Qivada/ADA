...
try:
    ...
    dfProcessDatetimes = spark.sql("SELECT CAST(date_sub(current_timestamp(), 90) AS timestamp) AS ArchiveDatetimeUTC")
...
for archiveLog in dfStaticArchiveLogs:
    ...
    if silverTableExists(__SILVER_PATH, __SILVER_TABLE_SCHEMA, __SILVER_TABLE_NAME) == False:
        ...
        dfSource.withColumn('__StartDatetimeUTC', lit(archiveLog.ArchiveDatetimeUTC)) \
        ...
        print(f"> Insert changed records into silver table {__SILVER_TABLE_FULLY_QUALIFIED_NAME}")
        # Get all current records i.e. the ones having MAX(__StartDatetimeUTC)
        dfTargetCurrent = spark.sql("""
        SELECT  {bkColumns}, 
                TRG.__HashDiff
        FROM    {targetTable} AS TRG
                INNER JOIN (
                    SELECT  {bkColumns},
                            MAX(__StartDatetimeUTC) AS __StartDatetimeUTC
                    FROM    {targetTable} AS TRG
                    GROUP BY {bkColumns}
                ) AS TRG_CURRENT ON {matchCondition}
        """.format(bkColumns = getColumnsWithAlias(__SILVER_TABLE_BK_COLUMNS, 'TRG'), \
                   targetTable = __SILVER_TABLE_FULLY_QUALIFIED_NAME, \
                   matchCondition = getMatchCondition(__SILVER_TABLE_BK_COLUMNS, "Match business keys", "TRG", "TRG_CURRENT") + " AND TRG.__StartDatetimeUTC = TRG_CURRENT.__StartDatetimeUTC"))

        # Filter dfSource to records that have changed compared to current records
        dfSourceChanges = dfSource.join(dfTargetCurrent, ([x.replace('`', '') for x in __SILVER_TABLE_BK_COLUMNS] + ["__HashDiff"]), "left_anti")

        # Append changed records
        dfSourceChanges.withColumn('__StartDatetimeUTC', lit(archiveLog.ArchiveDatetimeUTC)) \
                       .withColumn('__ArchiveDatetimeUTC', lit(archiveLog.ArchiveDatetimeUTC)) \
                       .withColumn('__ArchiveFilePath', lit(archiveLog.ArchiveFilePath)) \
                       .withColumn('__OriginalStagingFileName', lit(archiveLog.OriginalStagingFileName)) \
                       .write.format("delta") \
    ...
    if processLogs:
        # Commit log after each archive file to minimize risk of handling archive record multiple times (could happen if processing multiple files at once and process fails before fully completing)
        print(f"> Commit CDC log into table {__SILVER_LOG_TABLE_FULLY_QUALIFIED_NAME}")
